...
class StampPlotter:
    ...
    def doTrimBorderPixels(self):
        print('Border pixels{}will be trimmed.'.format(' ' if self._doTrimBorderPixels else ' not '))
    ...
    def loadDrizzledStamps(self, targetObject, targetPar, pipelineVersion = 6.2) :
        ...
        for extName in self.stampHdus.keys() :
            self.stampHdus[extName] = { key : (astrofits.getdata(stampPath, ext=(extName, 1), header=True) if os.path.isfile(stampPath) else None) for key, stampPath in self.stampPaths.items() }
    ...
    def loadDirectCutouts(self, targetObject, targetPar, pipelineVersion = 6.2) :
        ...
        self.directCutouts = { key : (self.extractDirectCutout(self.directHdus[key][0], self.stampHdus['SCI'][key][1]) if (self.stampHdus['SCI'][key] is not None and self.directHdus[key] is not None) else None) for key, directHdu in self.directHdus.items() }
    ...
    def plotZerothOrders(self, zerothOrderData, targetAxes, grism) :
        stampHeader = self.stampHdus['SCI'][grism][1]
        for region in zerothOrderData[grism].zerothOrderRegions :
            region.coord_list[0] -= stampHeader['CRPIX2']
            region.coord_list[1] -= stampHeader['CRPIX1']

        regionGraphics = zerothOrderData[grism].zerothOrderRegions.get_mpl_patches_texts()
        ...
        if self.stampHdus[extName] is not None :
            ...
            for stampIndex, (grism, hduData) in enumerate(self.stampHdus[extName].items()) :
                ...
                if self.doTrimBorderPixels :
                    cutoutData, cutoutModel, modelRect = self.getTrimmedStampData(stampData, modelData, grism, wcs=wcsObject)
                ...
                # Using aspect='auto' will force the subplots to stretch to fill the available space.
                ...
                if zerothOrderData is not None :
        ...
        if self.directCutouts is not None :
            ...
            for stampIndex, (grism, cutoutData) in enumerate(self.directCutouts.items()) :
                ...
                mplplot.title('Field {}, Object {}:\nDirect cutout for F{} (G{})'.format(self.targetPar,
                                                                                         self.targetObject,
                                                                                         self.getDirectFilterForGrism(grism),
                                                                                         grism))
                arcsecYAxis = subplotAxes.twinx()
                arcsecYAxis.set_ylim(*(np.array(subplotAxes.get_ylim()) - 0.5*np.sum(subplotAxes.get_ylim()))*self.directHdus[grism][1]['IDCSCALE'])
